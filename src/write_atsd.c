...
static int wa_write_messages(const data_set_t *ds, const value_list_t *vl,
    ...
    if ((now - cb->last_property_time) > WA_PROPERTY_INTERVAL) {
        ...
        if (!ret) {
            ...
            ssnprintf(sendline, sizeof(sendline), "property e:%s ms:%" PRIu64 " t:collectd-atsd v:host=%s\n",
    ...
    for (i = 0; i < ds->ds_num; i++) {
        ...
        if (strcasecmp(vl->plugin, "cpu") == 0) {
            ...
            if (strcasecmp(vl->type_instance, "idle") == 0) {
                ...
                if (same_value > 0) {
                    if (vl->plugin_instance[0] != '\0') {
                        ...
                        ssnprintf(sendline, sizeof(sendline), "series e:%s ms:%" PRIu64 " m:%s=%s\n",
        ...
        else if (starts_with("lvs.", vl->plugin)) {
            strlcat(metric_name, vl->plugin, sizeof(metric_name));
            if (cb->wa_num_caches > 0) {
                ak = (atsd_key_t *) malloc(sizeof(*ak));
                if (ak == NULL) {
            ...
            if (same_value > 0) {
                ssnprintf(sendline, sizeof(sendline), "series e:%s ms:%" PRIu64 " m:%s=%s t:volume_group=%s t:logical_volume=%s\n",
                        entity, CDTIME_T_TO_MS(vl->time), metric_name, ret, vl->plugin_instance, vl->type_instance);
                status = wa_send_message(sendline, cb);
                if (status != 0){
                    sfree(rates);
                    return (status);
